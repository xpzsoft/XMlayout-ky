...
$.xmlayout = {
	...
	layout : function(){
		var xmlayout = {
			...
			init : function(obj){
				...
				try{
					if(n_panel == undefined || n_panel == null)
						throw "XMlayout info: param 'panel' in function $.xmlayout init() is null!";

					if($.xmlayout.__isNaN(n_panel.height()) || $.xmlayout.__isNaN(n_panel.width()) || n_panel.height() <= 0 || n_panel.width() <= 0)
					...
					if(n_panel.children().length > 0){
						n_panel.children().remove();
					}
				}
				catch(err){
					console.error(err);
				...
				n_panel.append(panel);

				obj_this.random_class = "xm" + Math.random().toString().substr(2);
				obj_this.f_random_class = "." + obj_this.random_class;
				...
				$.xmlayout.panel_playuseable_arr[panel.prop("id")] = obj_this.playuseable;
			...
			__load : function(panel, url, data, fun, title, style){
				...
				if(title != undefined && title != null){
					var div = panel.parent().find('.targ_title');
					if(div[0]){
						div.children().remove();
			...
			__addRightPanel : function(panel, rp, tb, indexp){
				...
				if(tb){
					...
					panel.prop("mindex1", "#" + mid);
					panel.css("overflow", "hidden");
					var toolbar = $('<div id="' + mid + '" class="' + obj_this.random_class + '" style = "float:right; right:10px; margin-top:5px; width:140px; height:25px;">'
					...
					toolbar.children().eq(3).append($.xmlayout.__getPNGs(2));
					toolbar.children().eq(4).append($.xmlayout.__getPNGs(1));
				...
				var upanel = $('<div class="targ_panel" style="width:100%; height:100%; background-color: white; border-radius : 4px; overflow: auto;"></div>');
				if(tb){
					panel.append($('<div class="targ_title" style="width:100%; height: 30px; border-bottom : 0.5px solid #E3E3E3;"></div>'));
				...
				upanel.draw = function(){
					if($(this).find(".xmlayout-in")[0]){
						...
						if(tag.attr("type") == null || tag.attr("type") == "inherit")
							...
							console.error("XMlayout info : unknow value of 'type' [" + tag.attr("type") + "] which should be [auto], [inherit] and [self-def]!");
						}
				...
				if(rp){
					...
					var rp = $('<div id="' + mid + '" class="' + obj_this.random_class + '" style = "z-index:10000000; left:0px; top:0px; width:100px; height:auto; position:fixed; display:none;"> <ul class="list-group" style="margin-top: 0; padding-left: 0;">'
				...
				if(rp || tb){
					panel.find(obj_this.f_random_class).find(".xm-list-group-item").mousedown(function(e){
						...
						switch(action){
							case 0:
								...
								if(pp_node.parent.parent == null){
									var mlis = panel.find(obj_this.f_random_class).find(".fullscreen");
							...
							case 3:
								...
								if($(this).attr("popped") == "false"){
									...
									var mlis = panel.find(obj_this.f_random_class).find(".popped");
									...
									mlis = panel.find(obj_this.f_random_class).find(".fullscreen1");
									mlis.attr("fullscreen", "false");
									for(var i = 0; i < mlis.length; i++){
										if(mlis.eq(i).attr("type") != "button")
							...
							case 4:
								...
								if($(this).attr("fullscreen") == "false"){
									...
									var mlis = panel.find(obj_this.f_random_class).find(".fullscreen1");
									mlis.attr("fullscreen", "true");
									for(var i = 0; i < mlis.length; i++){
										if(mlis.eq(i).attr("type") != "button")
											mlis.eq(i).find("span").text($.xmlayout.LANG.rs);
						...
						if(values.length != rows){
							...
							for(var i = 0; i < rows; i++){
								...
								for(var j = 0; j < values[i].length; j++){
									if($.xmlayout.__isNaN(values[i][j]) || values[i][j] < 0){
			...
			__design : function(rows, cols, datas, panel, istext, isline){
				...
				if($.xmlayout.__isNaN(rows)){
					console.error("XMlayout info: 'rows' in function $.xmlayout __design() is not a number!");
					return;
				}
				if($.xmlayout.__isNaN(cols)){
					console.error("XMlayout info: 'cols' in function $.xmlayout __design() is not a number!");
					return;
				}
				...
				if(datas != null){
					if(!$.isArray(datas)){
						console.error("XMlayout info: 'datas' in function $.xmlayout __design() is not a array!");
						return;
					}
					if(datas.length != rows){
						console.error("XMlayout info: rows of 'datas' in function $.xmlayout __design() is not equal with '" + rows + "'!");
						return;
					}
					$.each(datas, function(i, v){
						if(!$.isArray(v)){
							console.error("XMlayout info: 'datas[" + i + "]' in function $.xmlayout __design() is not a array!");
							return;
						}
						if(v.length != cols + 1){
							console.error("XMlayout info: rows of 'datas[" + i + "]' in function $.xmlayout __design() is not equal with '" + (cols + 1)+ "'!");
							return;
						}
						...
						$.each(v, function(ii, vv){
							if(ii == 0){
								if(typeof vv == "number"){
									if(vv <= 0){
										console.error("XMlayout info: rows of 'datas[" + i + "," + ii + "]' in function $.xmlayout __design() is " + vv + ", but it should be > 0 !");
										return;
									}
									max[0].v += vv;
								}
								...
								if(typeof vv == "number"){
									...
									if($.trim(vv) == "auto"){
										console.error("XMlayout info: rows of 'datas[" + i + "," + ii + "]' in function $.xmlayout __design() is " + vv + ", but it should not be 'auto' !");
										return;
									}
									else
										tp += parseInt(vv);
	...
	__playOpen : function(){
		...
		for(var i = 0; i < obj_this.play_panel.length; i++){
			...
			obj_this.play_panel[i].css({position: "absolute", zIndex: 10000001, left: left + "%", top: top + "%", width: width + "%", height: height + "%"});
		}
		...
		$.dcdc0xdc = 0;
		$.dtdt0xdt = setInterval('if($.dcdc0xdc == 5){$.xmlayout.playbottom.children().css("display", "none");clearInterval($.dtdt0xdt);}else $.dcdc0xdc++;', 1000);
	...
	__bottom : function(){
		var obj_this = this;
		var div = $('<div style="position: fixed; width: 100%; height: 60px; bottom:-40px; z-index: 10000003;">'
				+ '<center>'
				+ '<div style="width: 80px; height: 20px; background-color: #848484; border-radius: 4px;"></div>'
	...
	addPlay : function(panels){
		if(panels.length == undefined || panels.length == null){
			console.error("XMlayout info: param 'panels' is not a array in function addPlay(panels)!");
			return;
		}
	...
	__clearInvalidObjects : function(){
		...
		for(var i = 0; i < obj_this.panel_ids.length; i++){
			if(!$("#" + obj_this.panel_ids[i])[0]){
				...
				if(obj_this.panel_playuseable_arr[obj_this.panel_ids[i]])
					obj_this.panel_playuseable--;
				i--;
			}
		}

		if(obj_this.panel_playuseable == 0){
