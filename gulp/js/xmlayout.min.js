...
$.xmlayout = {
	...
	layout : function(){
		var xmlayout = {
			...
			pop_count : [],
			popped_count : 0,
			RPID : {ZOOM_IN : 0, ZOOM_OUT : 1, FULLSCREEN_RESIZE : 2, POPPED_BERTH : 3, FULLSCREEN_RESIZE1 : 4},
			LANG : null,
			...
			init : function(obj){
				...
				try{
					...
					if(obj_this.__isNaN(panel.height()) || obj_this.__isNaN(panel.width()) || panel.height() <= 0 || panel.width() <= 0)
					...
					if(drag_bar_unit != undefined && drag_bar_unit != null){
						if(obj_this.__isNaN(drag_bar_unit))
					...
					obj_this.LANG = obj_this.__getLang(lang);

					if(popw != undefined && popw != null){
						if(obj_this.__isNaN(popw))
					...
					if(isborder != undefined && isborder != null){
						if(typeof isborder != "boolean")
							throw "XMlayout info: param 'isborder' in function $.xmlayout init() is not a boolean!";
					}
					else{
						isborder = true;
					}
				...
				obj_this.panel_offset = panel.offset();
				...
				obj_this.panel_tree = {panel : panel, lcpanel : null, rcpanel : null, parent : null};
				panel.prop({zoom : false, popped : 0, id : "id0"});
				obj_this.panel_tree_list["id0"] = obj_this.panel_tree;
				obj_this.panel_tree_list_i.push(obj_this.panel_tree);
				obj_this.__createLayout(data, panel, 1, obj_this.panel_tree);//绘制布局
				if(obj_this.playuseable){
					var mbody = $(window.document.body);
					...
					if($(".xmlayout_anim").length > 0)
						$(".xmlayout_anim").remove();
					if(mbody.prop("playbottom") != null)
						mbody.prop("playbottom").remove();
					if(mbody.prop("playheader") != null)
						mbody.prop("playheader").remove();
					obj_this.__play(panel);
			...
			__createLayout : function(data, panel, deep, panel_tree){
				...
				if(data.lr.length == 2){
					...
					var mv2 = 100 - mv1;
					var div1 = $('<div style="width:' + mv1 + '%; height:100%; float:left;"></div>');
					div1.prop({zoom : false, popped : 0, pvalue:"width", key:1, ovalue : mv1});
					var div2 = $('<div style="width:' + mv2 + '%; height:100%; float:left; border-radius: 4px;"></div>');
					...
					panel.append(div1);
					panel.append(div2);
					div1.prop("id", "id" + Math.random().toString());
					div2.prop("id", "id" + Math.random().toString());
					panel_tree.lcpanel = {panel : div1, lcpanel : null, rcpanel : null, parent : panel_tree};
					panel_tree.rcpanel = {panel : div2, lcpanel : null, rcpanel : null, parent : panel_tree};
					obj_this.panel_tree_list[div1.prop("id")] = panel_tree.lcpanel;
					obj_this.panel_tree_list[div2.prop("id")] = panel_tree.rcpanel;
					obj_this.panel_tree_list_i.push(panel_tree.lcpanel);
					obj_this.panel_tree_list_i.push(panel_tree.rcpanel);
					...
					var mv2 = 100 - mv1;
					var div1 = $('<div style="height:' + mv1 + '%; width:100%;"></div>');
					div1.prop({zoom : false, popped : 0, pvalue:"height", key:1, ovalue : mv1});
					var div2 = $('<div style="height:' + mv2 + '%; width:100%; border-radius: 4px;"></div>');
					...
					if(panel.prop("id") != "id0")
						panel.parent().css("background-color", obj_this.drag_bar_color);
					
					if(panel.prop("id") != "id0" && (data.rp || data.tb)){//如果没有布局，则禁止右键菜单、工具条
						obj_this.__addRightPanel(panel, data.rp, data.tb, data.indexp);
					}
					else{
						panel.prop("role", "targ_panel");//标明为目标容器
						panel.prop("indexp", data.indexp);
						obj_this.panel_arr.push(panel);
						panel.loadPage = function(url, data, fun, title, style){
			...
			__addListener : function(panel){
				...
				panel.on("dragstop", function(e){
					...
					for(var i = 0; i < obj_this.resize_fun.length; i++){
						if(obj_this.resize_params[i]!=null)
							obj_this.resize_fun[i](obj_this.resize_params[i]);
						else
							obj_this.resize_fun[i]();
					}
				});
			...
			__load : function(panel, url, data, fun, title, style){
				...
				{
					var div = $('<div style="height: 35px; background-color: #F0F0F0; border-top-left-radius: 4px; border-top-right-radius: 4px;"></div>');
					if(style != undefined && style != null && style.bgcolor != undefined && style.bgcolor!= null)
						div.css("background-color", style.bgcolor);
					var div1 = $('<div style="width:100%; height:calc(100% - 35px); overflow: auto; border-bottom-left-radius: 4px; border-bottom-right-radius: 4px;"></div>');
					if(style != undefined && style != null && style.pos == "center"){
						div.append($('<div style="height: 5px;"></div><center><label class="xmlayout_tt" style="font-size:15px;">' + title + '</label></center>'));
					}
					else{
						div.append($('<div style="height: 5px;"></div><label class="xmlayout_tt" style="font-size:15px;margin-left: 5px;">' + title + '</label>'));
					}
					if(style != undefined && style != null && style.fontsize != undefined && style.fontsize!= null){
						...
						div.find("div").css("height", (30 - style.fontsize)/2 + "px");
					}
				...
				if(url == null || url == undefined){
					console.error("XMlayout info: param 'url' in function loadPage() is null!");
					return;
				}
				tgd.load(url, data, fun);
			},
			__addRightPanel : function(panel, rp, tb, indexp){
				var obj_this = this;
				
				if(tb){
					panel.prop("mindex1", "#xmlayout_tb" + obj_this.panel_arr.length);
					panel.css("overflow", "hidden");
					var toolbar = $('<div id="xmlayout_tb' + obj_this.panel_arr.length + '" class="xmlayout" style = "float:right; right:10px; margin-top:5px; width:140px; height:30px;">'
						+ '<div type="button" class="xm-list-group-item popped" action=3 popped="false" value=3 style="height:30px; width: 20px; float:right; opacity:0.25; margin-right:5px;"></div>'
						+ '<div type="button" class="xm-list-group-item fullscreen1" action=4 fullscreen="false" value=4 style="height:30px; width: 20px; float:right; display:none; opacity:0.25; margin-right:5px;"></div>'
						+ '<div type="button" class="xm-list-group-item fullscreen" action=2 fullscreen="false" value=2 style="height:30px; width: 20px; float:right; opacity:0.25; margin-right:5px;"></div>'
						+ '<div type="button" class="xm-list-group-item zoom-out" action=1 value=1 style="height: 30px; width: 20px; float:right; opacity:0.25; margin-right:5px; display:none;"></div>'
						+ '<div type="button" class="xm-list-group-item zoom-in" action=0 value=0 style="height: 30px; width: 20px; float:right; opacity:0.25; margin-right:5px;"></div>'
						+ '</div>');
					panel.append(toolbar);
					toolbar.children().eq(0).append(obj_this.__getPNGs(4));
					toolbar.children().eq(0).append(obj_this.__getPNGs(5));
					toolbar.children().eq(1).append(obj_this.__getPNGs(3));
					toolbar.children().eq(2).append(obj_this.__getPNGs(3));
					toolbar.children().eq(3).append(obj_this.__getPNGs(2));
					toolbar.children().eq(4).append(obj_this.__getPNGs(1));
				...
				var upanel = $('<div class="targ_panel" style="width:100%; height:100%; background-color: white; border-radius : 4px;"></div>');
				upanel.prop("role", "targ_panel");//标明为目标容器
				upanel.prop("indexp", indexp);
				panel.append(upanel);
				...
				if(rp){
					panel.prop("mindex", "#xmlayout_rp" + obj_this.panel_arr.length);
					var rp = $('<div id="xmlayout_rp' + obj_this.panel_arr.length + '" class="panel panel-default xmlayout" style = "z-index:10000000; left:0px; top:0px; width:100px; height:auto; position:fixed; display:none;"> <ul class="list-group" style="margin-top: 0; padding-left: 0;">'
							+ '<li class="xm-list-group-item zoom-in" action=0 value=0 style="position: relative;display: block;padding: 5px 15px;margin-bottom: -1px;background-color: #fff;border: 1px solid #ddd;"><span style="cursor:default; font-size:12px;">' + obj_this.LANG.zi + '</span></li>'
							+ '<li class="xm-list-group-item zoom-out" action=1 value=1 style="position: relative;display: none;padding: 5px 15px;margin-bottom: -1px;background-color: #fff;border: 1px solid #ddd;"><span style="cursor:default; font-size:12px;">' + obj_this.LANG.zo + '</span></li>'
							+ '<li class="xm-list-group-item fullscreen" action=2 fullscreen="false" value=2 style="position: relative;display: block;padding: 5px 15px;margin-bottom: -1px;background-color: #fff;border: 1px solid #ddd;"><span style="cursor:default;font-size:12px;">' + obj_this.LANG.fs + '</span></li>'
							+ '<li class="xm-list-group-item fullscreen1" action=4 fullscreen="false" value=4 style="position: relative;display: block;padding: 5px 15px;margin-bottom: -1px;background-color: #fff;border: 1px solid #ddd; display:none;"><span style="cursor:default;font-size:12px;">' + obj_this.LANG.ff + '</span></li>'
							+ '<li class="xm-list-group-item popped" action=3 popped="false" value=3 style="position: relative;display: block;padding: 5px 15px;margin-bottom: -1px;background-color: #fff;border: 1px solid #ddd;"><span style="cursor:default;font-size:12px;">' + obj_this.LANG.pp + '</span></li>'
							+ '</ul></div>');
					panel.append(rp);
				...
				panel.find(".xmlayout").find(".xm-list-group-item").mousedown(function(e){
					...
					switch(action){
						case 0:
							...
							if(pp_node.parent.parent == null){
								var mlis = panel.find(".xmlayout").find(".fullscreen");
								mlis.attr("fullscreen", "true");
								for(var i = 0; i < mlis.length; i++){
									if(mlis.eq(i).attr("type") != "button")
										mlis.eq(i).find("span").text(obj_this.LANG.rs);
									else
										mlis.eq(i).find("img").attr("title", obj_this.LANG.rs);
								}
								obj_this.disablePanelItem(panel.find("div.targ_panel"), obj_this.RPID.ZOOM_IN);
							}
							obj_this.useablePanelItem(panel.find("div.targ_panel"), obj_this.RPID.ZOOM_OUT);
						...
						case 1:
							...
							for(var i = 0; i < mlis.length; i++){
								if(mlis.eq(i).attr("type") != "button")
									mlis.eq(i).find("span").text(obj_this.LANG.fs);
								else
									mlis.eq(i).find("img").attr("title", obj_this.LANG.fs);
							}

							if(pp_node.lcpanel == null && pp_node.rcpanel == null){
								obj_this.disablePanelItem(panel.find("div.targ_panel"), obj_this.RPID.ZOOM_OUT);
							}
							obj_this.useablePanelItem(panel.find("div.targ_panel"), obj_this.RPID.ZOOM_IN);
						...
						case 3:
							...
							if($(this).attr("popped") == "false"){
								...
								var mlis = panel.find(".xmlayout").find(".popped");
								mlis.attr("popped", "true");
								for(var i = 0; i < mlis.length; i++){
									if(mlis.eq(i).attr("type") != "button")
										mlis.eq(i).find("span").text(obj_this.LANG.bt);
								}

								mlis = panel.find(".xmlayout").find(".fullscreen1");
								mlis.attr("fullscreen", "false");
								for(var i = 0; i < mlis.length; i++){
									if(mlis.eq(i).attr("type") != "button")
										mlis.eq(i).find("span").text(obj_this.LANG.ff);
									else
										mlis.eq(i).find("img").attr("title", obj_this.LANG.ff);
								}
								...
								obj_this.useablePanelItem(panel.find("div.targ_panel"), obj_this.RPID.FULLSCREEN_RESIZE1);
								...
								for(var i = 0; i < mlis.length; i++){
									if(mlis.eq(i).attr("type") != "button")
										mlis.eq(i).find("span").text(obj_this.LANG.pp);
								}
								obj_this.popped_count--;
								...
								obj_this.disablePanelItem(panel.find("div.targ_panel"), obj_this.RPID.FULLSCREEN_RESIZE1);
								var xds = $(this).siblings();
								for(var i = 0; i < xds.length; i++){
									if(xds.eq(i).attr("action") == 4){
										xds.eq(i).attr("fullscreen", "false");
										if($(this).attr("type") != "button")
											xds.eq(i).find("span").text(obj_this.LANG.ff);
										break;
									}
								}
							}
						...
						case 4:
							...
							if($(this).attr("fullscreen") == "false"){
								...
								pp_node.panel.css("width", "100%");
								pp_node.panel.css("height", "100%");
								pp_node.panel.css("left", "0px");
								pp_node.panel.css("top", "0px");
								pp_node.panel.draggable( "option", "disabled", true);
								
								var mlis = panel.find(".xmlayout").find(".fullscreen1");
								mlis.attr("fullscreen", "true");
								for(var i = 0; i < mlis.length; i++){
									if(mlis.eq(i).attr("type") != "button")
										mlis.eq(i).find("span").text(obj_this.LANG.rs);
			...
			__drawp2 : function(panel_tree){
				if(panel_tree.lcpanel != null){
					...
					if(panel.css("position") != "absolute"){
						...
						panel.css("border", "1px solid #C2C2C2");
						...
						panel.resize(function(argument) {
							for(var i = 0; i < obj_this.resize_fun.length; i++){
								if(obj_this.resize_params[i]!=null)
										obj_this.resize_fun[i](obj_this.resize_params[i]);
									else
						...
						if(rpp.prop("mindex")!=undefined && rpp.prop("mindex")!=null){
							obj_this.disablePanelItem(rpp.find("div.targ_panel"), obj_this.RPID.ZOOM_IN);
							obj_this.disablePanelItem(rpp.find("div.targ_panel"), obj_this.RPID.FULLSCREEN_RESIZE);
						}
			...
			__drawleafnode : function(panel, pvalue){
				...
				if(panel.css("position") == "absolute"){
					...
					// if(rpp.prop("mindex")!=undefined && rpp.prop("mindex")!=null){
					// 	obj_this.useablePanelItem(rpp.find("div.targ_panel"), obj_this.RPID.ZOOM_IN);
					// 	obj_this.useablePanelItem(rpp.find("div.targ_panel"), obj_this.RPID.FULLSCREEN_RESIZE);
					// }
				}
			},
			__getLang : function(lang){
				if(lang != "chs" && lang != "en"){
					console.error("XMlayout info: param 'lang' is invalid in $.xmlayout function __getLang()!");
					return;
				}
				if(lang == "chs"){
					var obj = {
			...
			__play : function(panel){
				var obj_this = this;
				$(window).keydown(function(e){
					var code = e.which || e.keyCode;
					if(obj_this.__isNaN(code))
					{
						console.error("XMlayout info: can't get the keyCode in function keydown()!");
						return;
					}
					if(code == 17)
						$(this).prop("ctrl", true);
					...
					if($(this).prop("ctrl") && code == 81){
						if(!obj_this.anim_tf){
							...
							$(document).scrollLeft(0);console.log($("*"));
			...
			__playOpen : function(){
				...
				if(obj_this.ispop){
					for(var i = 0; i < obj_this.panel_arr.length; i++){
						var pp = obj_this.panel_arr[i].parent();
						if(pp.prop("zoom") == undefined || pp.prop("zoom") == null)
							pp = pp.parent();
						if(pp.prop("ispopped")){
				...
				if(obj_this.iszoom){
					for(var i = 0; i < obj_this.panel_tree_list_i.length; i++){
						if(obj_this.panel_tree_list_i[i].panel.css("display") == "none"){
							obj_this.panel_tree_list_i[i].panel.prop("odisplay", "none");
							obj_this.panel_tree_list_i[i].panel.css("display", "block");
						}
					}
				}
				...
				if(obj_this.play_panel.length == 0){
					for(var i = 0; i < obj_this.panel_arr.length; i++){
						obj_this.play_panel.push(obj_this.panel_arr[i]);
					}
				}
				for(var i = 0; i < obj_this.play_panel.length; i++){
					obj_this.play_panel[i].css("position", "absolute");
					obj_this.play_panel[i].css("z-index", 10000001);
					var left = 5 + 100 * i;
					var top = 5;
					var width = 90, height = 90;
					obj_this.play_panel[i].css("left", left + "%");
					obj_this.play_panel[i].css("top", top + "%");
					obj_this.play_panel[i].css("width", width + "%");
					obj_this.play_panel[i].css("height", height + "%");
				}
				...
				$(window).click(function(){
					obj_this.playbottom.css("display", "block");
				});
			},
			__playTurnoff : function(obj_this){
				...
				for(var i = 0; i < obj_this.play_panel.length; i++){
					obj_this.play_panel[i].css("position", "static");
					obj_this.play_panel[i].css("z-index", 0);
					obj_this.play_panel[i].css("width", "100%");
					obj_this.play_panel[i].css("height", "100%");
				}
				...
				if(obj_this.iszoom){
					for(var i = 0; i < obj_this.panel_tree_list_i.length; i++){
						if(obj_this.panel_tree_list_i[i].panel.prop("odisplay") == "none"){
							obj_this.panel_tree_list_i[i].panel.prop("odisplay", "block");
							obj_this.panel_tree_list_i[i].panel.css("display", "none");
						}
					}
				}
			...
			__header : function(){
				var div = $('<div style="position: fixed; width: 180px; height: auto; left:calc(50% - 90px); left:-webkit-calc(50% - 105px); left:-moz-calc(50% - 105px); top:10px; z-index: 10000003;">'
      				+ '<center>'
        				+ '<div style="width: 80px; height: 20px; background-color: white; border-radius: 5px;"><span style="font-size:16px; font-weight:bold;">1/' + obj_this.panel_arr.length + '</span></div>'
        			+ '</center>'
        		+ '</div>');
        		div.prop('np', 1);
        		div.prop('ap', obj_this.panel_arr.length);
        		div.css("display", "none");
        		return div;
			},
			__bottom : function(){
				var obj_this = this;
				var div = $('<div style="position: fixed; width: 180px; height: auto; left:calc(50% - 90px); left:-webkit-calc(50% - 105px); left:-moz-calc(50% - 105px); bottom:-40px; z-index: 10000003;">'
      				+ '<center>'
        				+ '<div style="width: 80px; height: 20px; background-color: #A8A8A8; border-radius: 4px;"></div>'
        				+ '<div style="width: 168px; height: 38px; background-color: #A8A8A8; border-radius: 4px;"><div style="margin-left:23px;"></div></div>'
        			+ '</center>'
        		+ '</div>');
        		...
        		var div2 = $('<div class="xmlayout_anim" style="position:absolute; left:0px; top:0px; width:100%; height:100%; z-index:10000000; background-color:black; opacity:1.0;"></div>');
				var div1 = $('<div class="xmlayout_anim" style="position:absolute; left:0px; top:0px; width:100%; height:100%; z-index:10000002; opacity:0.0;"></div>');
				...
				img7.click(function(e){
        			if(obj_this.anim_wd == null){
	        			...
	        			obj_this.anim_wd = setInterval("obj_this.__anim(0)", obj_this.timer);
				...
				$(window).keydown(function(e){
					if(obj_this.anim_tf){
						...
						if(e.which != undefined && e.which != null)
							...
							console.error("can't get the keyCode in function keydown()!");
							return;
						}
						if(code == 37){
			...
			getPanels : function(indexp){
				if(indexp == null || indexp == undefined)
					...
					for(var i = 0; i < this.panel_arr.length; i++){
						if(indexp == this.panel_arr[i].prop("indexp")){
			...
			setPlay : function(panels){
				if(panels.length == undefined || panels.length == null){
					console.error("XMlayout info: param 'panels' is not a array in function setPlay(panels)!");
					return;
				}
				...
				for(var i = 0; i < panels.length; i++){
					if(panels[i].prop("role") != "targ_panel"){
						console.error("XMlayout info: the panel whoes index is " + i + " in array param 'panels' is invalid !");
						continue;
					}
					this.play_panel.push(panels[i]);
				}

				obj_this.playheader.prop("ap", this.play_panel.length);
				obj_this.playheader.find("span").text("1/" + this.play_panel.length);
			},
			element : function(value, resize, rp, tb, lrs, uds){
				var obj_this = this;
				try{
					if(obj_this.__isNaN(value))
						throw "XMlayout info: 'value' in function $.xmlayout element() is not a number!";
			...
			design : function(rows, cols, rc, values, attr, node){
				var obj_this = this;
				if(obj_this.__isNaN(rows)){
					console.error("XMlayout info: 'rows' in function $.xmlayout design() is not a number!");
					return;
				}
				if(obj_this.__isNaN(cols)){
					console.error("XMlayout info: 'cols' in function $.xmlayout design() is not a number!");
					return;
				}
				if(rc != 0 && rc != 1){
					rc = 0;
				...
				if(rows < 1){
					console.error("XMlayout info: 'rows' in function $.xmlayout design() should be bigger than 0!");
					return;
				}
				if(cols < 1){
					console.error("XMlayout info: 'cols' in function $.xmlayout design() should be bigger than 0!");
					return;
				}
				...
				if(values != null){
					if(rc == 0){
						if(values.length != rows){
							console.error("XMlayout info: 'values' length in function $.xmlayout design() is invalid, it should be equal with 'rows'=" + rows + "!");
							return;
						}
						else{
							for(var i = 0; i < rows; i++){
								if(values[i].length != cols + 1){
									console.error("XMlayout info: 'values[" + i + "]' length in function $.xmlayout design() is invalid, it should be equal with 'cols'+1=" + cols + 1 + "!");
									return;
								}
								for(var j = 0; j < values[i].length; j++){
									if(obj_this.__isNaN(values[i][j]) || values[i][j] < 0){
										console.error("XMlayout info: 'values[" + i + "," + j + "]' length in function $.xmlayout design() is invalid!");
										return;
									}
								}
							}
						...
						if(values.length != cols){
							console.error("XMlayout info: 'values' length in function $.xmlayout design() is invalid, it should be equal with 'cols'=" + cols + "!");
							return;
						}
						else{
							for(var i = 0; i < cols; i++){
								if(values[i].length != rows + 1){
									console.error("XMlayout info: 'values[" + i + "]' length in function $.xmlayout design() is invalid, it should be equal with 'rows'+1=" + rows + 1 + "!");
									return;
								}
								for(var j = 0; j < values[i].length; j++){
									if(obj_this.__isNaN(values[i][j]) || values[i][j] < 0){
				...
				function setAttr(node, attr){
					...
					for(var i = 0; i < attr.length; i++){
						if(attr[i].length != 3){
							console.error("XMlayout info: 'attr' in function $.xmlayout design() is out off index!");
							return;
						}
				...
				function calcData(rows, cols, values, data, rc){
					var pd = data;
			...
			resize : function(fun, params){
				this.resize_fun.push(fun);
				this.resize_params.push(params);
				this.resize_ids.push("rid" + Math.random());
				return this.resize_ids[this.resize_ids.length - 1];
			},
			removeResize : function(rid){
				for(var i = 0; i < this.resize_ids.length; i++){
					if(this.resize_ids[i] == rid){
						this.resize_fun.splice(i,1);
						this.resize_params.splice(i,1);
						this.resize_ids.splice(i,1);
						break;
					}
				}
			},
			addRightPanelItem : function(panel, text, fun){
				if(panel.parent() == null  || panel.prop("role") != "targ_panel"){
					console.error("XMlayout info: param [panel] is invalid in function $.xmlayout addRightPanelItem() !");
					return;
				}
			...
			removeRightPanelItem : function(panel, id){
				if(panel.parent() == null  || panel.prop("role") != "targ_panel"){
					console.error("XMlayout info: param 'panel' is invalid in function $.xmlayout removeRightPanelItem() !");
					return;
				}
			...
			disablePanelItem : function(panel, id, isinit){
				if(panel.parent() == null  || panel.prop("role") != "targ_panel"){
					console.error("XMlayout info: param 'panel' is invalid in function $.xmlayout disablePanelItem() !");
					return;
				}
			...
			useablePanelItem : function(panel, id, isinit){
				if(panel.parent() == null  || panel.prop("role") != "targ_panel"){
					console.error("XMlayout info: param 'panel' is invalid in function useablePanelItem() !");
					return;
				}
	...
	addParam : function(datas){
		if(datas == null || datas == undefined || typeof datas != 'object'){
			console.error("XMlayout info: param 'data' is not a object in function addParam() !");
			return;
		}
		var obj_this = this;
